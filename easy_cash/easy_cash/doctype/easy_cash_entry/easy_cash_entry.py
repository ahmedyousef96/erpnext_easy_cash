...
class EasyCashEntry(AccountsController):
    ...
    def validate_category_lines(self):
        ...
        all_categories = list(
            set(row.category for row in self.category_lines if row.category)
        )
        if all_categories:
    ...
    def validate_treasury(self):
        if not self.treasury:
            return
        treasury_data = frappe.db.get_value(
            "Treasury", self.treasury, ["company", "disabled"], as_dict=True
        )
        ...
        for row in self.category_lines:
            if row.account:
                ...
                if account_currency and account_currency != self.currency:
                    frappe.throw(
                        _(
                            "Row {0}: Account {1} currency ({2}) does not match entry currency ({3})"
                        ).format(row.idx, row.account, account_currency, self.currency),
    ...
    def get_gl_entries(self):
        ...
        against_accounts = ", ".join(
            set(row.account for row in self.category_lines if row.account)
        )
